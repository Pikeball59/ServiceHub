...
class Contact(models.Model):
    ...
    user = models.ForeignKey(User, verbose_name='Пользователь',
                             ...
                             on_delete=models.CASCADE)
    type = models.CharField(verbose_name='Тип контакта', choices=TYPE_CHOICES, max_length=10)
    # Поля для адреса
    ...
    def save(self, *args, **kwargs):
        # Валидация: телефон должен быть заполнен, если тип 'phone'
        if self.type == 'phone':
            if not self.phone:
                raise ValidationError('Для типа "Телефон" необходимо указать номер.')
            # Очищаем поля адреса
            self.city = self.street = self.house = self.structure = self.building = self.apartment = ''
        elif self.type == 'address':
            if not (self.city and self.street and self.phone):
                raise ValidationError('Для типа "Адрес" необходимо указать город, улицу и телефон.')
        else:
            raise ValidationError('Неверный тип контакта.')

        # Проверка лимитов: телефон – не более 1, адрес – не более 5
        existing = Contact.objects.filter(user=self.user, type=self.type)
        if self.pk:
            existing = existing.exclude(pk=self.pk)
        if self.type == 'phone' and existing.count() >= 1:
            raise ValidationError('У пользователя может быть только один телефонный контакт.')
        if self.type == 'address' and existing.count() >= 5:
            raise ValidationError('У пользователя может быть не более пяти адресов.')
